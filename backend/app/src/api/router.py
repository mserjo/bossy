...
from .external import external_api_router # Імпортуємо external_api_router з api/external/__init__.py
...
try:
    # from .v1 import v1_router # Вже імпортовано вище
...
try:
    # external_api_router вже імпортовано вище
    api_router.include_router(external_api_router, prefix="/external", tags=["External API (Webhooks)"])
    logger.info("Роутер для зовнішніх API (вебхуків) успішно підключено до api_router.")
except ImportError as e:
    logger.warning(f"Не вдалося імпортувати external_api_router з api.external: {e}. Зовнішні API (вебхуки) не будуть доступні.")
    pass
