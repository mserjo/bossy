...
@router.post(
    ...
    description="Аутентифікує користувача за email (в полі username) та паролем, повертає access та refresh токени." # i18n
)
async def login_for_access_token(
    ...
    return TokenResponse(
        ...
        refresh_token=refresh_token_str, # Повертаємо також у тілі для зручності клієнтів, що не можуть використовувати cookies
        token_type="bearer"
    )
...
async def get_user_from_valid_refresh_token_cookie( # Залежність для /refresh та /logout
    ...
    if not user: # Якщо process_refresh_token повертає None при помилці валідації
        ...
        # i18n
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Недійсний або прострочений refresh token.")
    return user
...
@router.post(
    ...
    description="Оновлює токен доступу, використовуючи refresh token з HttpOnly cookie. Реалізує ротацію refresh token." # i18n
)
...
@router.post(
    ...
    description="Здійснює вихід користувача, інвалідуючи поточний refresh token (видаляючи його з БД та cookie)." # i18n
)
async def logout(
    response: Response,
    ...
    if success:
        ...
        logger.warning(f"Не вдалося відкликати refresh token (можливо, вже не існував або невалідний): ...{refresh_token_str[-6:]}")
