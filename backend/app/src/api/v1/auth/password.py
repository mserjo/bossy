...
async def change_password_endpoint( # Перейменовано, щоб уникнути конфлікту з імпортом PasswordService
    ...
    try:
        ...
        if not success: # change_password може повертати False або кидати ValueError
            # i18n
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Не вдалося змінити пароль. Перевірте поточний пароль.")
...
async def forgot_password_endpoint( # Перейменовано
    ...
    if user and user.is_active:
        ...
        logger.info(f"Токен скидання паролю для {user.email}: {reset_token}. Посилання: {frontend_reset_url}") # Тільки для розробки!
        ...
        # )
        logger.warning(f"[ЗАГЛУШКА] Надсилання email для скидання пароля користувачу {user.email} з посиланням: {frontend_reset_url}")
    else:
        logger.info(f"Користувача з email '{request_data.email}' не знайдено або він неактивний. Надсилання email не відбувається.")
...
async def reset_password_endpoint( # Перейменовано
    ...
    try:
        ...
        if not success:
            ...
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Не вдалося скинути пароль. Можливо, токен недійсний, прострочений або користувач не знайдений.")
        ...
        logger.error(f"Неочікувана помилка скидання пароля (токен: {reset_data.token[:8]}...): {e}", exc_info=global_settings.DEBUG)
        # i18n
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Виникла помилка сервера при спробі скинути пароль.")
