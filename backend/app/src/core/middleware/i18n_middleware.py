...
from backend.app.src.core.i18n import current_request_language # Added import

logger = get_logger(__name__)

SUPPORTED_LANGUAGES: List[str] = ["uk", "en"]  # Ukrainian first as preferred
DEFAULT_LANGUAGE: str = "uk"

# ContextVar is now imported from core.i18n
...
class LanguageMiddleware(BaseHTTPMiddleware):
    async def dispatch(
        ...
        lang_code = DEFAULT_LANGUAGE # Default language

        accept_language_header: Optional[str] = request.headers.get("Accept-Language")

        if accept_language_header:
            # Simple parsing: take the first preferred language from the header
            # Example: "uk-UA,uk;q=0.9,en-US;q=0.8,en;q=0.7"
            # More sophisticated parsing can be done using libraries like `negotiator_rs` or `babel`.
            # For now, a basic approach:
            languages = [lang.split(';')[0].strip() for lang in accept_language_header.split(',')]
            for lang in languages:
                main_lang_part = lang.split('-')[0].lower() # e.g., "uk" from "uk-UA"
                if main_lang_part in SUPPORTED_LANGUAGES:
                    ...
                    break
            else: # If no supported language found in header
        ...
        # Store the determined language code in request.state to be accessible
        # by other parts of the application (like dependency injectors or directly in route handlers if needed).
        ...
        # You can also set a "Content-Language" header in the response
