...
Цей модуль визначає `BaseDictionary`, який успадковує `BaseMainModel`
...
class BaseDictionary(BaseMainModel):
    """
...
if __name__ == "__main__":
    ...
    logger.info("--- Базова модель для довідників (BaseDictionary) ---")
    logger.info(f"Клас BaseDictionary: {BaseDictionary}")
    logger.info(f"Абстрактний: {getattr(BaseDictionary, '__abstract__', False)}")
    ...
    for attr in expected_attrs_from_basemainmodel:
        logger.info(f"  - {attr}")
    logger.info("\nВласні атрибути BaseDictionary:")
    for attr in expected_attrs_own:
        logger.info(f"  - {attr}")

    # Приклад того, як __repr__ може виглядати для моделі, що успадковує BaseDictionary
    # (спрощена імітація без реальної інженерії SQLAlchemy)
    from backend.app.src.models.base import Base
    from datetime import datetime


    class DummyStatus(BaseDictionary):
        ...
        # _repr_fields = [] # Це б означало, що тільки поля з BaseDictionary та його предків будуть в repr
