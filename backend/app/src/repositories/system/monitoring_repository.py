...
if __name__ == "__main__":
    ...
    print("--- Репозиторії Системного Моніторингу ---")

    print(f"\n--- Репозиторій Системних Логів ({SystemLogRepository.__name__}) ---")
    print(f"Модель: {SystemLog.__name__}, Схема створення: {SystemLogCreateSchema.__name__}")
    print("Специфічні методи:")
    print("  - get_logs_by_level(level: str, ...)")
    print("  - get_logs_by_source(source: str, ...)")
    print("  - get_logs_for_user(user_id: int, ...)")
    print("  - clear_old_logs(before_timestamp: datetime)")
    print("TODO: Інтегрувати Enum 'LogLevel' для аргументу `level`.")

    print(f"\n--- Репозиторій Метрик Продуктивності ({PerformanceMetricRepository.__name__}) ---")
    print(f"Модель: {PerformanceMetric.__name__}, Схема створення: {PerformanceMetricCreateSchema.__name__}")
    print("Специфічні методи:")
    print("  - get_metrics_by_name(metric_name: str, ...)")
    print("  - get_metrics_by_tag(tag_key: str, tag_value: str, ...)")
    print("  - clear_old_metrics(before_timestamp: datetime)")
    print("TODO: Реалізувати `get_metrics_by_tag` з урахуванням діалекту БД для JSON запитів.")
