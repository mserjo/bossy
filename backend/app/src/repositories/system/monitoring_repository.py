...
class SystemLogRepository(BaseRepository[SystemLog, SystemLogCreateSchema, SystemLogUpdateSchema]):
    ...
    async def get_logs_by_level(
        ...
        Args:
            ...
            level (LogLevel): Рівень логу (Enum).
        ...
        """
        logger.debug(f"Отримання логів за рівнем: {level.value}, skip: {skip}, limit: {limit}")
        # Модель SystemLog.level очікує LogLevel (SQLEnum), тому передаємо Enum член напряму.
        ...
        try:
            ...
            logger.debug(f"Знайдено {total_count} логів за рівнем: {level.value if isinstance(level, Enum) else level}") # Log enum value
            return items, total_count
        except Exception as e:
            logger.error(f"Помилка при отриманні логів за рівнем {level.value if isinstance(level, Enum) else level}: {e}", exc_info=True) # Log enum value
            return [], 0
...
if __name__ == "__main__":
    ...
    logger.info("  - get_logs_by_level(level: LogLevel, ...)") # Оновлено тип
    ...
    # logger.info("TODO: Інтегрувати Enum 'LogLevel' для аргументу `level`.") # Вирішено
    ...
    # from datetime import datetime  # datetime вже імпортовано на початку файлу
