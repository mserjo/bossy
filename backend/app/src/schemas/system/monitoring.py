...
from backend.app.src.core.dicts import LogLevel
...
class SystemLogResponseSchema(SystemLogBaseSchema, IDSchemaMixin):
    """
...
class PerformanceMetricResponseSchema(PerformanceMetricBaseSchema, IDSchemaMixin):
    """
...
# --- Схеми для фільтрів запитів ---

class SystemLogQueryFiltersSchema(BaseSchema):
    """
    Схема для параметрів фільтрації запитів системних логів.
    """
    level: Optional[LogLevel] = Field(None, description=_("system.monitoring.log.filters.level.description"))
    source: Optional[str] = Field(None, max_length=LOG_SOURCE_MAX_LENGTH, description=_("system.monitoring.log.filters.source.description"))
    user_id: Optional[int] = Field(None, description=_("system.monitoring.log.filters.user_id.description"))
    message_contains: Optional[str] = Field(None, description=_("system.monitoring.log.filters.message_contains.description"))
    start_date: Optional[datetime] = Field(None, description=_("system.monitoring.log.filters.start_date.description"))
    end_date: Optional[datetime] = Field(None, description=_("system.monitoring.log.filters.end_date.description"))
    # model_config успадковується з BaseSchema


class PerformanceMetricQueryFiltersSchema(BaseSchema):
    """
    Схема для параметрів фільтрації запитів метрик продуктивності.
    """
    metric_name: Optional[str] = Field(None, max_length=METRIC_NAME_MAX_LENGTH, description=_("system.monitoring.metric.filters.metric_name.description"))
    start_date: Optional[datetime] = Field(None, description=_("system.monitoring.metric.filters.start_date.description"))
    end_date: Optional[datetime] = Field(None, description=_("system.monitoring.metric.filters.end_date.description"))
    # Теги (tags) обробляються окремо як JSON рядок в ендпоінті, але можна розглянути додавання сюди
    # tags: Optional[Dict[str, str]] = Field(None, description="Фільтр за тегами")
    # model_config успадковується з BaseSchema


class SystemMonitoringSummaryResponseSchema(BaseSchema):
    """
    Схема для зведеної інформації моніторингу системи.
    """
    recent_error_count: int = Field(description=_("system.monitoring.summary.fields.recent_error_count.description"))
    critical_performance_issues: int = Field(description=_("system.monitoring.summary.fields.critical_performance_issues.description"))
    last_log_entry_at: Optional[datetime] = Field(None, description=_("system.monitoring.summary.fields.last_log_entry_at.description"))
    last_metric_recorded_at: Optional[datetime] = Field(None, description=_("system.monitoring.summary.fields.last_metric_recorded_at.description"))
    status_message: str = Field(description=_("system.monitoring.summary.fields.status_message.description"))
    # model_config успадковується з BaseSchema
...
if __name__ == "__main__":
    ...
    log_response_instance = SystemLogResponseSchema(**log_response_data)
    ...
    metric_response_instance = PerformanceMetricResponseSchema(**metric_response_data)
