...
if __name__ == "__main__":
    ...
    print("--- Pydantic Схеми для Завдань (Task) ---")
    ...
    print(create_instance.model_dump_json(indent=2, exclude_none=True))
    ...
    print(task_response_instance.model_dump_json(indent=2, exclude_none=True))
    ...
    print(task_detail_instance.model_dump_json(indent=2, exclude_none=True))
    ...
    print("Також, `task_type_code` та `status_code` потребують валідації на рівні сервісу або схеми.")
    print("Узгодження `status_code` та `state` залишається відкритим питанням (TODO).")
