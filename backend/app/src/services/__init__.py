...
# --- Cache Services ---
BASE_CACHE_SERVICE = "BaseCacheService"
REDIS_CACHE_SERVICE = "RedisCacheService"
IN_MEMORY_CACHE_SERVICE = "InMemoryCacheService"

_cache_service_names = [
    BASE_CACHE_SERVICE, REDIS_CACHE_SERVICE, IN_MEMORY_CACHE_SERVICE
]
_imported_cache_services = {}

try:
    from .cache import (
        BaseCacheService, RedisCacheService, InMemoryCacheService
    )
    _imported_cache_services[BASE_CACHE_SERVICE] = BaseCacheService
    _imported_cache_services[REDIS_CACHE_SERVICE] = RedisCacheService
    _imported_cache_services[IN_MEMORY_CACHE_SERVICE] = InMemoryCacheService
    logger.info("Successfully imported all specified cache services.")
except ImportError as e:
    logger.warning(f"Could not import one or more cache services from .cache: {e}. Attempting individual imports.")
    for service_name_const in _cache_service_names:
        try:
            module = __import__("app.src.services.cache", globals(), locals(), [service_name_const], 0)
            _imported_cache_services[service_name_const] = getattr(module, service_name_const)
            logger.info(f"Successfully imported {service_name_const} individually.")
        except (ImportError, AttributeError) as ie:
            logger.warning(f"Could not import {service_name_const} from .cache: {ie}")
            _imported_cache_services[service_name_const] = None

for name, service in _imported_cache_services.items():
    if service: globals()[name] = service
    else: globals()[name] = None # Ensure name exists as None if not imported
...
__all__.extend([name for name, service in _imported_cache_services.items() if service is not None])
