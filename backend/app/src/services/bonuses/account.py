...
class UserAccountService(BaseService):
    ...
    async def get_user_account(
        ...
        """
        log_msg = f"користувача ID '{user_id}'"
        if group_id: log_msg += f" в групі ID '{group_id}'"
        else: log_msg += " (глобальний)"
    ...
    async def get_or_create_user_account(
        ...
        """
        account_db = await self._get_user_account_orm(user_id, group_id, load_relations=True) # Завантажуємо відносини для можливого повернення
        if account_db:
            return account_db
        ...
        try:
            ...
            if account_db_retry:
                logger.info(f"Рахунок для {log_msg} знайдено після помилки цілісності (ймовірно, створено конкурентно).")
    ...
    async def adjust_account_balance(
        ...
        if adjustment_amount < Decimal("0.00") and current_balance < abs(adjustment_amount):
            logger.warning(f"Недостатньо коштів для {log_msg}. Поточний баланс: {current_balance}, спроба списання: {abs(adjustment_amount)}.")
        ...
        await self.db_session.refresh(transaction_db, attribute_names=['account']) # Завантажити зв'язок з рахунком для транзакції

        logger.info(f"Баланс для {log_msg} (ID Рахунку: {account_db.id}) скориговано на {adjustment_amount}. Новий баланс: {new_balance}.")
    ...
    async def list_user_accounts(
        ...
        """
        logger.debug(f"Перелік бонусних рахунків: user={user_id}, group={group_id}, global_only={filter_global_only}, min_balance={min_balance}, skip={skip}, limit={limit}")

        stmt = select(UserAccount).options(
            selectinload(UserAccount.user).options(selectinload(User.user_type)),
            selectinload(UserAccount.group) # Завжди намагаємось завантажити, SQLAlchemy впорається, якщо group_id є None
        )
