...
class _CacheEntry:
    ...
    """
    def __init__(self, value: Any, expire_at: Optional[float] = None): # expire_at - це Unix timestamp від time.monotonic()
...
class InMemoryCacheService(BaseCacheService):
    ...
    async def set(self, key: str, value: Any, expire_seconds: Optional[int] = None) -> bool:
        ...
        if expire_seconds is not None:
            if expire_seconds <= 0: # Якщо термін дії не позитивний, видаляємо ключ
                logger.debug(f"Кеш SET: ключ='{key}' з непозитивним expire_seconds ({expire_seconds}). Видалення, якщо існує.")
        ...
        logger.debug(f"Кеш SET: ключ='{key}', значення='{str(value)[:100]}...', expire_at (монотонний): {expire_at}, приблизний UTC час закінчення: {log_expire_str}")
        return True
    ...
    async def increment(self, key: str, amount: int = 1) -> Optional[int]:
        ...
        if current_entry:
            if current_entry.is_expired():
                ...
                logger.error(f"Кеш INCREMENT помилка для ключа '{key}': існуюче значення не є цілим числом (тип: {type(current_entry.value)}). Інкремент неможливий.")
    ...
    async def decrement(self, key: str, amount: int = 1) -> Optional[int]:
        ...
        if current_entry:
            if current_entry.is_expired():
                ...
                logger.error(f"Кеш DECREMENT помилка для ключа '{key}': існуюче значення не є цілим числом (тип: {type(current_entry.value)}). Декремент неможливий.")
                return None
    ...
    async def set_add(self, key: str, *values: Any) -> int:
        ...
        if entry and not entry.is_expired():
            if isinstance(entry.value, set):
                ...
                logger.warning(f"Кеш SADD: ключ '{key}' існує, але не є множиною (тип: {type(entry.value)}). Буде перезаписано новою множиною.")
        ...
        logger.debug(f"Кеш SADD: ключ='{key}', {added_count} нових значень додано. Поточний розмір: {len(current_set)}.")
        return added_count
    ...
    async def set_get_all(self, key: str) -> Set[Any]:
        ...
        if entry_value is not None: # Якщо ключ існує, але не є множиною
            logger.warning(f"Кеш SMEMBERS: ключ '{key}' існує, але не є множиною (тип: {type(entry_value)}). Повернення порожньої множини.")
        return set()
    ...
    async def set_remove(self, key: str, *values: Any) -> int:
        ...
        if entry and not entry.is_expired() and isinstance(entry.value, set):
            ...
            logger.debug(f"Кеш SREM: ключ='{key}', {removed_count} значень видалено. Поточний розмір: {len(current_set)}.")
        else:
    ...
    async def set_is_member(self, key: str, value: Any) -> bool:
        ...
        logger.debug(f"Кеш SISMEMBER: ключ='{key}' не знайдено, прострочено, або не є множиною. Значення='{value}' не є членом.")
        return False
