...
class BaseDictionaryService(
    ...
    async def create(self, data: SchemaCreateType, **kwargs: Any) -> SchemaResponseType:
        ...
        try:
            ...
            raise ValueError(f"Не вдалося створити {self._model_name} через конфлікт даних. Перевірте унікальність полів.") # i18n
    ...
    async def update(self, item_id: UUID, data: SchemaUpdateType, **kwargs: Any) -> Optional[SchemaResponseType]:
        ...
        if 'name' in update_data and hasattr(self.model, 'name') and update_data['name'] != getattr(item_db, 'name'):
            stmt_name = select(self.model.id).where(self.model.name == update_data['name'], self.model.id != item_id) # type: ignore
            if (await self.db_session.execute(stmt_name)).scalar_one_or_none():
        ...
        try:
            ...
            raise ValueError(f"Не вдалося оновити {self._model_name} через конфлікт даних. Перевірте унікальність полів.") # i18n
    ...
    async def delete(self, item_id: UUID) -> bool:
        ...
        try:
            ...
            await self.commit()
            logger.info(f"{self._model_name} з ID '{item_id}' (Код: {getattr(item_db, 'code', 'N/A')}) успішно видалено.")
            return True
        except IntegrityError as e:
            await self.rollback()
            item_code = getattr(item_db, 'code', item_id)
            logger.error(f"Помилка цілісності при видаленні {self._model_name} '{item_code}': {e}. Можливо, елемент використовується.", exc_info=settings.DEBUG)
