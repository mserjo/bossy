...
class FileRecordService(BaseService):
    ...
    async def create_file_record(self, record_data: FileRecordCreate, uploader_user_id: Optional[UUID] = None) -> FileRecordResponse:
        """
    ...
    async def delete_file_record(
        ...
        try:
            ...
            await self.rollback()
            logger.error(f"Помилка цілісності при видаленні запису файлу ID '{file_id}': {e}. Можливо, він використовується.", exc_info=settings.DEBUG)
            # i18n
        ...
        if delete_from_storage and storage_path_to_delete:
            ...
            try:
                ...
                logger.error(f"Помилка при видаленні файлу '{storage_path_to_delete}' зі сховища: {e}. Запис в БД вже видалено.", exc_info=True) # i18n log
            ...
            logger.warning(f"Прапорець delete_from_storage встановлено, але шлях зберігання для запису ID '{file_id}' не визначено.")
    ...
    async def list_file_records(
        ...
        logger.debug(f"Перелік записів файлів: користувач={uploader_user_id}, група={group_id}, тип_сутності='{entity_type}', ID_сутності={entity_id}, MIME='{mime_type_pattern}'")

        query = select(FileRecord)
        options_to_load = []
        if hasattr(FileRecord, 'uploader_user'): options_to_load.append(selectinload(FileRecord.uploader_user).options(selectinload(User.user_type)))
