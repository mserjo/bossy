...
class UserLevelService(BaseService):
    ...
    async def _get_orm_user_level(self, user_id: UUID, group_id: Optional[UUID] = None, load_relations: bool = True) -> Optional[UserLevel]:
    ...
    async def get_user_level(
        ...
        else: log_ctx_parts.append("(глобальний контекст)")
    ...
    async def update_user_level(
        ...
        existing_user_level_db = await self._get_orm_user_level(user_id, group_id, load_relations=False) # Зв'язки не потрібні для логіки
        ...
        if not target_level_schema: # Балів недостатньо для будь-якого рівня
            ...
            if existing_user_level_db: # Якщо був попередній рівень, його треба видалити
                logger.info(f"Видалення існуючого запису UserLevel ID {existing_user_level_db.id} для {log_ctx}, оскільки балів недостатньо.")
        ...
        if existing_user_level_db: # Якщо запис UserLevel існує
            if existing_user_level_db.level_id == target_level_schema.id:
                ...
                logger.info(f"Оновлення рівня для {log_ctx} з ID '{existing_user_level_db.level_id}' на новий ID '{target_level_schema.id}' ('{target_level_schema.name}').")
            ...
            logger.info(f"Створення нового запису UserLevel для {log_ctx} на рівні ID '{target_level_schema.id}' ('{target_level_schema.name}').")
