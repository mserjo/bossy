...
class GroupService(BaseService):
    ...
    async def get_group_by_id(self, group_id: UUID, include_details: bool = False) -> Optional[
    ...
    async def update_group(self, group_id: UUID, group_update_data: GroupUpdate, current_user_id: UUID) -> Optional[
        GroupDetailedResponse]:
    ...
    async def delete_group(self, group_id: UUID, current_user_id: UUID) -> bool:
        ...
        try:
            ...
            await self.rollback()
            logger.error(f"Помилка цілісності '{group_id}': {e}. Можливо, група використовується.",
                         exc_info=settings.DEBUG)
            # i18n
