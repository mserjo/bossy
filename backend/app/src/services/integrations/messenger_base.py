...
class MessengerUserProfile(BaseModel):
    ...
    """
    id: str = Field(..., description="ID користувача, специфічний для платформи (наприклад, Telegram chat_id, Slack user_id)") # i18n
...
class MessageSendCommand(BaseModel):
    ...
    """
    recipient_platform_id: str = Field(..., description="ID отримувача, специфічний для платформи (ID користувача або каналу)") # i18n
...
class MessageSendResponse(BaseModel):
    ...
    platform_message_id: Optional[str] = Field(None, description="ID повідомлення на зовнішній платформі, якщо є") # i18n
...
class BaseMessengerIntegrationService(BaseService, ABC):
    ...
    def __init__(self, db_session: AsyncSession, user_id_for_context: Optional[UUID] = None):
        ...
        logger.info(f"BaseMessengerIntegrationService (підклас: {self.__class__.__name__}) ініціалізовано для користувача: {self.user_id_for_context or 'N/A'}.")
