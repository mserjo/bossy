...
MICROSOFT_AUTH_URL = getattr(settings, 'MICROSOFT_AUTH_URL', "https://login.microsoftonline.com/common/oauth2/v2.0/authorize")
MICROSOFT_TOKEN_URL = getattr(settings, 'MICROSOFT_TOKEN_URL', "https://login.microsoftonline.com/common/oauth2/v2.0/token")
...
class OutlookCalendarService(BaseCalendarIntegrationService):
    ...
    async def _get_user_tokens_from_db(self, user_id: UUID) -> Optional[Dict[str, Any]]:
        ...
        if mock_tokens:
            logger.warning(f"[ЗАГЛУШКА] _get_user_tokens_from_db (Outlook) викликано для {user_id}. Симуляція отримання.")
            return mock_tokens
    ...
    async def _store_user_tokens_in_db(self, user_id: UUID, tokens: Dict[str, Any], account_identifier: str) -> bool:
        ...
        logger.info(f"[ЗАГЛУШКА] _store_user_tokens_in_db (Outlook) викликано для {user_id} з ID '{account_identifier}'. Симуляція збереження.")
    ...
    async def _get_graph_api_client(self, user_id: UUID) -> Optional[Any]: # Повинен повертати тип клієнта, напр. GraphServiceClient
        """
        ...
        if not await self.refresh_access_token_if_needed(user_id):
             logger.warning(f"Не вдалося забезпечити актуальність токенів Microsoft Graph API для користувача {user_id}.")
             return None
    ...
    async def connect_account(self, user_id: UUID, auth_code: str, redirect_uri: str) -> Dict[str, Any]:
        ...
        logger.info(f"OutlookCalendar: Користувач {user_id} підключається з auth_code (довжина: {len(auth_code)}) через {redirect_uri}.")
        ...
        if stored:
            ...
            # i18n
            return {"status": "success", "message": "Outlook Calendar успішно підключено (симуляція).", "account_identifier": account_id_sim}
        else:
    ...
    async def refresh_access_token_if_needed(self, user_id: UUID) -> bool:
        ...
        account_id = getattr(self, f"_mock_account_id_user_{user_id}_{self.service_name}", f"user_ms_{str(user_id)[:4]}@outlook.com")
    ...
    async def list_user_calendars(self, user_id: UUID) -> List[CalendarInfo]:
        ...
        return [
            CalendarInfo(id=f"primary_outlook_cal_id_{uuid4()}", name="Календар", is_primary=True, can_edit=True), # i18n
            CalendarInfo(id=f"work_outlook_cal_id_{uuid4()}", name="Робочі події", is_primary=False, can_edit=True) # i18n
        ]
    ...
    async def create_event(self, user_id: UUID, calendar_id: str, event_data: CalendarEventData) -> Optional[CalendarEventData]:
        ...
        logger.info(f"OutlookCalendar: Користувач {user_id} створює подію в календарі '{calendar_id}': '{event_data.title}'.")
    ...
    async def get_event(self, user_id: UUID, calendar_id: str, event_id: str) -> Optional[CalendarEventData]:
        ...
        logger.info(f"[ЗАГЛУШКА] OutlookCalendar: Отримання події '{event_id}' для {user_id} в календарі '{calendar_id}'.")
    ...
    async def update_event(self, user_id: UUID, calendar_id: str, event_id: str, event_data: CalendarEventData) -> Optional[CalendarEventData]:
        ...
        logger.info(f"[ЗАГЛУШКА] OutlookCalendar: Оновлення події '{event_id}' для {user_id} в '{calendar_id}' з назвою '{event_data.title}'.")
        # Pydantic v2: model_dump()
    ...
    async def delete_event(self, user_id: UUID, calendar_id: str, event_id: str) -> bool:
        ...
        logger.info(f"[ЗАГЛУШКА] OutlookCalendar: Видалення події '{event_id}' для {user_id} в календарі '{calendar_id}'.")
        return True
    ...
    async def list_events(self, user_id: UUID, calendar_id: str, start_time: datetime, end_time: datetime, query: Optional[str] = None) -> List[CalendarEventData]:
        ...
        logger.info(f"[ЗАГЛУШКА] OutlookCalendar: Перелік подій для {user_id}, календар '{calendar_id}', запит: '{query}'.")
        ...
        if event1_start < effective_end_time:
            # i18n
            mock_events.append(CalendarEventData(id=f"mock_outlook_event1_{uuid4()}", title="Робоча зустріч (Outlook)", start_time=event1_start, end_time=event1_start + timedelta(hours=1)))
        if event2_start < effective_end_time:
            # i18n
             mock_events.append(CalendarEventData(id=f"mock_outlook_event2_{uuid4()}", title="Особиста зустріч (Outlook)", start_time=event2_start, end_time=event2_start + timedelta(hours=2)))
        return mock_events
