...
from backend.app.src.models.notifications.delivery import \
    NotificationDeliveryAttempt  # Модель SQLAlchemy DeliveryAttempt
...
class NotificationDeliveryService(BaseService):
    ...
    def __init__(self, db_session: AsyncSession):
        ...
        self.channel_senders: Dict[str, ChannelSender] = {
            ...
            # Припускаємо, що EmailNotificationService не потребує db_session в конструкторі
    ...
    async def queue_notification_for_delivery(self, notification_id: UUID, user_id: UUID) -> List[
    ...
    async def send_notification_to_user(
        ...
        if not notification_db.user:  # Користувач має бути завантажений через selectinload
            logger.error(
                f"Користувача ID '{user_id}' для сповіщення ID '{notification_id}' не знайдено (або не завантажено). Надсилання неможливе.")
            return []
        ...
        for channel_key in channels_to_try:
            ...
            logger.debug(
                f"Спроба доставки сповіщення ID '{notification_id}' користувачу ID '{user_id}' через канал '{channel_key}'.")
        ...
        for attempt_db in attempt_orm_objects:
            ...
            try:
                ...
                logger.info(
                    f"Спроба доставки ID '{attempt_db.id}' для сповіщення ID '{notification_id}' через {attempt_db.channel}: Статус {attempt_db.status}, ID Повід.: {attempt_db.external_message_id}")
            except Exception as e:
                logger.error(
                    f"Виняток під час спроби надсилання ID '{attempt_db.id}' для сповіщення '{notification_id}' через {attempt_db.channel}: {e}",
                    exc_info=True)
        ...
        logger.info(
            f"Завершено всі спроби доставки для сповіщення ID '{notification_id}' користувачу ID '{user_id}'. {len(attempt_orm_objects)} спроб оброблено.")
    ...
    async def retry_failed_delivery_attempts(self, max_attempts: int = 3, age_threshold_hours: int = 24) -> int:
        ...
        stmt = select(NotificationDeliveryAttempt).options(
            ...
            NotificationDeliveryAttempt.status.in_(["FAILED", "RETRY_SCHEDULED"]),
            # Можливо, є статус для запланованих повторів
        ...
        for attempt in failed_attempts_db:
            ...
            logger.info(
                f"Повторна спроба доставки для сповіщення ID '{attempt.notification_id}' (Спроба ID: {attempt.id}) користувачу ID '{attempt.notification.user.id}' через канал '{attempt.channel}'. Попередніх спроб: {attempt.attempt_count}.")

            sender = self.channel_senders.get(attempt.channel)
            if not sender:
                logger.error(
                    f"Немає відправника для каналу '{attempt.channel}' спроби ID {attempt.id}. Повтор неможливий.")
            ...
            try:
                ...
                if new_retry_attempt_db.status == "FAILED":
                    new_retry_attempt_db.error_message = str(
                        send_result.get("error", "Помилка повторної відправки"))  # i18n
                else:  # Успішний повтор
                    retried_success_count += 1
                logger.info(
                    f"Результат повторної спроби {new_retry_attempt_db.attempt_count} для Сповіщ. ID '{attempt.notification_id}' через {attempt.channel}: Статус {new_retry_attempt_db.status}")
            except Exception as e:
                logger.error(
                    f"Виняток під час повторного надсилання для Сповіщ. ID '{attempt.notification_id}' через {attempt.channel}: {e}",
                    exc_info=True)
        ...
        if failed_attempts_db:  # Якщо були спроби для обробки
            await self.commit()
            logger.info(
                f"Оброблено {len(failed_attempts_db)} невдалих спроб доставки. Успішних повторів: {retried_success_count}.")
