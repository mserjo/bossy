...
DEFAULT_TASK_TYPES = [
    ...
    {"name": "Складне Завдання", "code": "COMPLEX",
     "description": "Завдання, що може вимагати кількох кроків або більших зусиль."},  # i18n
    {"name": "Подія", "code": "EVENT", "description": "Запланована подія або активність."},  # i18n
    {"name": "Доручення", "code": "CHORE",
     "description": "Повторюване або рутинне завдання, часто пов'язане з побутом."},  # i18n
...
DEFAULT_BONUS_TYPES = [
    ...
    {"name": "Бонус за Досягнення", "code": "ACHIEVEMENT_BONUS",
     "description": "Бонус, що надається за конкретні досягнення."},  # i18n
    {"name": "Штрафні Бали", "code": "PENALTY_POINTS", "description": "Бали, що списуються як штраф.",
     "is_penalty": True},  # i18n
]
...
DEFAULT_STATUSES = [
    ...
    {"name": "Завершено", "code": "COMPLETED", "entity_type": "TASK", "description": "Завдання успішно виконано."},
    # i18n
    {"name": "Ухвалено", "code": "APPROVED", "entity_type": "TASK_COMPLETION",
     "description": "Виконання завдання ухвалено."},  # i18n
    {"name": "Відхилено", "code": "REJECTED", "entity_type": "TASK_COMPLETION",
     "description": "Виконання завдання відхилено."},  # i18n
    {"name": "Активний", "code": "ACTIVE", "entity_type": "USER",
     "description": "Обліковий запис користувача активний."},  # i18n
    {"name": "Неактивний", "code": "INACTIVE", "entity_type": "USER",
     "description": "Обліковий запис користувача неактивний."},  # i18n
    {"name": "В очікуванні", "code": "PENDING", "entity_type": "INVITATION",
     "description": "Запрошення очікує на відповідь."},  # i18n
    {"name": "Прийнято", "code": "ACCEPTED", "entity_type": "INVITATION", "description": "Запрошення прийнято."},
    # i18n
]
...
DEFAULT_SYSTEM_USERS = [
    ...
    },
    {
        ...
        "user_type_code": "BOT_TYPE", "user_role_codes": ["ADMIN"]
        # Бот може мати адмінські права для системних завдань
    },
    {
...
class InitialDataService(BaseService):
    ...
    async def _initialize_dictionary(self, model_cls: Type[Any], defaults: List[Dict[str, Any]],
                                     code_field: str = "code") -> int:
        """
        ...
        for item_data in defaults:
            ...
            if not code_value:
                logger.warning(
                    f"Пропуск елемента в {model_cls.__name__} через відсутність значення для поля '{code_field}': {item_data}")
                continue
            ...
            if not existing_item:
                try:
                    ...
                    logger.info(
                        f"Створено {model_cls.__name__} '{item_data.get('name', code_value)}' з кодом '{code_value}'.")
                    created_count += 1
    ...
    async def initialize_system_users(self) -> int:
        ...
        for user_data in DEFAULT_SYSTEM_USERS:
            ...
            try:
                ...
                if not user_type:
                    logger.error(
                        f"Тип користувача '{user_data['user_type_code']}' не знайдено для системного користувача '{username}'. Пропуск.")
                    continue
                ...
                if len(user_roles_db) != len(user_data.get("user_role_codes", [])):
                    ...
                    logger.warning(
                        f"Не всі ролі знайдено для '{username}'. Знайдено: {found_codes}, Відсутні: {missing_codes}.")
        ...
        logger.info(
            f"Завершено підготовку системних користувачів. Готово до створення: {created_count} нових користувачів.")
        return created_count
    ...
    async def _check_essential_data_exists(self) -> bool:
        ...
        su_role = (
            await self.db_session.execute(select(UserRole.id).where(UserRole.code == "SUPERUSER"))).scalar_one_or_none()
        su_type = (await self.db_session.execute(
            select(UserType.id).where(UserType.code == "SUPERUSER_TYPE"))).scalar_one_or_none()
        bot_type = (
            await self.db_session.execute(select(UserType.id).where(UserType.code == "BOT_TYPE"))).scalar_one_or_none()
        admin_role = (
            await self.db_session.execute(select(UserRole.id).where(UserRole.code == "ADMIN"))).scalar_one_or_none()

        if not all([su_role, su_type, bot_type, admin_role]):
            logger.warning(
                f"Перевірка критичних даних: Роль SUPERUSER: {'OK' if su_role else 'ВІДСУТНЯ'}, Тип SUPERUSER_TYPE: {'OK' if su_type else 'ВІДСУТНІЙ'}, Тип BOT_TYPE: {'OK' if bot_type else 'ВІДСУТНІЙ'}, Роль ADMIN: {'OK' if admin_role else 'ВІДСУТНЯ'}")  # i18n
            return False
        return True
    ...
    async def run_full_initialization(self) -> Dict[str, int]:
        ...
        try:
            ...
            if not await self._check_essential_data_exists():
                # i18n
                logger.error(
                    "Критично важливі типи/ролі для створення системних користувачів відсутні після ініціалізації довідників. Ініціалізація користувачів скасована.")
