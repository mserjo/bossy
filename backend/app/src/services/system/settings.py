...
class SystemSettingService(BaseService):
    ...
    async def create_setting(self, setting_data: SystemSettingCreate, creator_id: Optional[UUID] = None) -> SystemSettingResponse:
        """
        ...
        try:
            ...
            await self.rollback()
            logger.error(f"Помилка цілісності при створенні налаштування '{setting_data.key}': {e}", exc_info=global_settings.DEBUG)
            # i18n
    ...
    async def update_setting(self, setting_id: UUID, setting_update_data: SystemSettingUpdate, updater_id: Optional[UUID] = None) -> Optional[SystemSettingResponse]:
        """
        ...
        if 'key' in update_data and update_data['key'] != setting_db.key:
            ...
            if existing_key_setting and existing_key_setting.id != setting_id:
                ...
                logger.warning(f"Неможливо оновити ключ для ID '{setting_id}' на '{update_data['key']}', оскільки він використовується ID '{existing_key_setting.id}'. {msg}")
                raise ValueError(msg)
        ...
        try:
            ...
            await self.rollback()
            logger.error(f"Помилка цілісності при оновленні налаштування ID '{setting_id}': {e}", exc_info=global_settings.DEBUG)
            # i18n
    ...
    def _deserialize_setting_value(self, value_str: str, value_type: str) -> Any:
        ...
        try:
            ...
            logger.error(f"Помилка десеріалізації значення '{value_str}' як '{value_type}': {e}", exc_info=global_settings.DEBUG)
    ...
    async def get_setting_value(self, key: str, default: Optional[Any] = None) -> Optional[Any]:
        ...
        logger.info(f"Системне налаштування '{key}' не знайдено або значення/тип не встановлено. Повернення значення за замовчуванням: {default}")
        return default
