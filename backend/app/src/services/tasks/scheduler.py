...
class TaskSchedulingService(BaseService):
    ...
    async def process_recurring_tasks(self) -> List[UUID]:
        ...
        for template in recurring_templates_db:
            if await self._should_create_new_instance(template, now):
                try:
                    logger.info(f"Створення нового екземпляра для шаблону повторюваного завдання ID: {template.id} ('{template.title}')")
                    ...
                    if not new_due_date: # Якщо не вдалося розрахувати (напр. непідтримуваний патерн)
                        logger.warning(f"Не вдалося розрахувати наступну дату виконання для шаблону ID {template.id}. Пропуск.")
                        continue
                    ...
                    if not status_id_to_set or not await self.db_session.get(Status, status_id_to_set):
                        ...
                        if not open_status:
                            logger.error(f"Статус за замовчуванням '{DEFAULT_TASK_STATUS_CODE_OPEN}' не знайдено, неможливо створити екземпляр для шаблону {template.id}")
                            continue
                    ...
                    logger.info(f"Створено новий екземпляр завдання ID: {new_instance_model.id} для шаблону ID: {template.id}")
                    ...
                    logger.error(f"Помилка обробки шаблону повторюваного завдання ID {template.id}: {e}", exc_info=global_settings.DEBUG)
        ...
        if newly_created_task_ids:
            try:
                ...
                logger.info(f"Успішно оброблено повторювані завдання. Створено {len(newly_created_task_ids)} нових екземплярів.")
            except Exception as e:
    ...
    async def _should_create_new_instance(self, template_task: Task, current_time: datetime) -> bool:
        ...
        if template_task.recurrence_end_date and current_time > template_task.recurrence_end_date:
            logger.info(f"Шаблон повторюваного завдання ID {template_task.id} завершився {template_task.recurrence_end_date}. Новий екземпляр не створюється.")
            return False

        if not interval: # Не вказано інтервал повторення
            logger.warning(f"Для шаблону ID {template_task.id} не вказано інтервал повторення. Новий екземпляр не створюється.")
            return False
    ...
    async def _calculate_next_due_date(self, template_task: Task, reference_date: datetime) -> Optional[datetime]:
        ...
        instance_due_time = template_task.due_date.time() if template_task.due_date else datetime.min.time().replace(hour=23, minute=59, second=59)
        ...
        # ... і так далі для MONTHLY, YEARLY (потребує dateutil.relativedelta)
            ...
            logger.warning(f"Не вдалося точно розрахувати дату для інтервалу '{template_task.recurrence_interval}'. Використання +1 день.")
            next_day = (template_task.last_instance_created_at or reference_date).date() + timedelta(days=1)
    ...
    async def send_task_reminders(self) -> int:
        ...
        stmt = select(Task).join(Status, Task.status_id == Status.id).where(
            ...
            (Task.last_reminder_sent_at.is_(None) | (Task.last_reminder_sent_at < (now - timedelta(hours=23)))) # type: ignore
        ).options(
        ...
        for task in tasks_to_remind_db:
            if not task.assignments:
                logger.debug(f"Завдання ID {task.id} ('{task.title}') не має призначених виконавців. Пропуск нагадування.")
                continue

            for assignment in task.assignments:
                if assignment.is_active and assignment.user:
                    try:
                        logger.info(f"[ЗАГЛУШКА] Надсилання нагадування для завдання ID {task.id} ('{task.title}') користувачу ID {assignment.user_id}.")
                        ...
                        logger.error(f"Помилка надсилання нагадування для завдання ID {task.id} користувачу ID {assignment.user_id}: {e}", exc_info=global_settings.DEBUG)
                        # Не зупиняємо весь процес через помилку одного нагадування

        if reminders_sent_count > 0:
            try:
                await self.commit()
                logger.info(f"Успішно надіслано {reminders_sent_count} нагадувань про завдання та оновлено позначки часу.")
            except Exception as e:
