...
def main():
    ...
    # Цей скрипт буде просто передавати всі аргументи після себе до pytest.
    # parse_known_args() тут не потрібен, якщо ми не маємо власних аргументів скрипта.
    # Замість цього, ми можемо просто взяти всі аргументи sys.argv[1:].

    # Аргументи, що передаються скрипту (крім назви самого скрипта)
    pytest_run_args = sys.argv[1:]
