...
kudos/                                 # Кореневий каталог проекту
...
│   │   │   ├── kudos_api.postman_collection.json # JSON файл з колекцією запитів Postman для всіх ендпоінтів API
│   │   │   ├── kudos_environments.postman_environment.json # JSON файл з налаштуваннями середовищ Postman (dev, staging, prod)
...
│   ├── Dockerfile                   # Інструкції для Docker по збірці образу backend додатку
...
│   │       │   ├── redis.py         # Модуль для конфігурації підключення до Redis (для черг та кешування)
...
│   │       │   ├── mixins.py        # Модуль з SQLAlchemy міксинами для додавання спільної функціональності (TimestampedMixin, SoftDeleteMixin, NameDescriptionMixin, StateMixin, GroupAffiliationMixin, NotesMixin)
│   │       │   │
│   │       │   ├── system/          # Каталог для системних моделей
│   │       │   │   ├── __init__.py  # Ініціалізаційний файл пакету `system.models`
│   │       │   │   ├── settings.py  # SQLAlchemy модель для зберігання налаштувань системи (SystemSetting)
│   │       │   │   ├── monitoring.py# SQLAlchemy моделі для збору даних моніторингу (SystemLog, PerformanceMetric)
│   │       │   │   └── health.py    # SQLAlchemy моделі для health check (ServiceHealthStatus - якщо перевіряється стан залежних сервісів)
│   │       │   │
│   │       │   ├── dictionaries/    # Каталог для моделей довідників (кожен успадковує `BaseDictionaryModel`)
│   │       │   │   ├── __init__.py  # Ініціалізаційний файл пакету `dictionaries.models`
│   │       │   │   ├── base_dict.py # Базовий клас `BaseDictionaryModel` для моделей довідників (успадковує `BaseMainModel`, додає `code`)
│   │       │   │   ├── statuses.py  # SQLAlchemy модель для довідника "Статуси" (Status)
│   │       │   │   ├── user_roles.py# SQLAlchemy модель для довідника "Ролі користувачів" (UserRole)
│   │       │   │   ├── user_types.py# SQLAlchemy модель для довідника "Типи користувачів" (UserType)
│   │       │   │   ├── group_types.py# SQLAlchemy модель для довідника "Типи груп" (GroupType)
│   │       │   │   ├── task_types.py# SQLAlchemy модель для довідника "Типи завдань" (TaskType)
│   │       │   │   ├── bonus_types.py# SQLAlchemy модель для довідника "Типи бонусів" (BonusType)
│   │       │   │   ├── calendars.py # SQLAlchemy модель для довідника "Календарі" (CalendarProvider)
│   │       │   │   └── messengers.py# SQLAlchemy модель для довідника "Месенджери" (MessengerPlatform)
│   │       │   │
...
│   │       │   │   ├── user.py      # SQLAlchemy модель "Користувач" (User), успадковує `BaseMainModel`
│   │       │   │   ├── token.py     # SQLAlchemy модель для зберігання Refresh токенів (RefreshToken)
│   │       │   │   └── session.py   # SQLAlchemy модель для сесій користувачів (UserSession)
│   │       │   │
│   │       │   ├── groups/          # Каталог для моделей, пов'язаних з групами
│   │       │   │   ├── __init__.py  # Ініціалізаційний файл пакету `groups.models`
│   │       │   │   ├── group.py     # SQLAlchemy модель "Група" (Group), успадковує `BaseMainModel`
│   │       │   │   ├── settings.py  # SQLAlchemy модель для налаштувань конкретної групи (GroupSetting), пов'язана з `Group`
│   │       │   │   ├── membership.py# SQLAlchemy модель "Членство в групі" (GroupMembership), зв'язок user-group з роллю
│   │       │   │   └── invitation.py# SQLAlchemy модель "Запрошення до групи" (GroupInvitation)
│   │       │   │
│   │       │   ├── tasks/           # Каталог для моделей, пов'язаних із завданнями та подіями
│   │       │   │   ├── __init__.py  # Ініціалізаційний файл пакету `tasks.models`
│   │       │   │   ├── task.py      # SQLAlchemy модель "Завдання" (Task), успадковує `BaseMainModel`
│   │       │   │   ├── event.py     # SQLAlchemy модель "Подія" (Event), успадковує `BaseMainModel`
│   │       │   │   ├── assignment.py# SQLAlchemy модель "Призначення завдання/події" (TaskAssignment)
│   │       │   │   ├── completion.py# SQLAlchemy модель "Виконання завдання/події" (TaskCompletion)
│   │       │   │   └── review.py    # SQLAlchemy модель "Відгук на завдання" (TaskReview)
│   │       │   │
│   │       │   ├── bonuses/         # Каталог для моделей, пов'язаних з бонусами та рахунками
│   │       │   │   ├── __init__.py  # Ініціалізаційний файл пакету `bonuses.models`
│   │       │   │   ├── bonus.py     # SQLAlchemy модель "Бонус" (BonusRule), опис правила нарахування/штрафу для завдання/події
│   │       │   │   ├── account.py   # SQLAlchemy модель "Рахунок користувача" (UserAccount)
│   │       │   │   ├── transaction.py# SQLAlchemy модель "Транзакція по рахунку" (AccountTransaction)
│   │       │   │   └── reward.py    # SQLAlchemy модель "Нагорода" (Reward), успадковує `BaseMainModel`
│   │       │   │
│   │       │   ├── gamification/    # Каталог для моделей, пов'язаних з геймифікацією
│   │       │   │   ├── __init__.py  # Ініціалізаційний файл пакету `gamification.models`
│   │       │   │   ├── level.py     # SQLAlchemy модель "Рівень" (Level) - як довідник або налаштування
│   │       │   │   ├── user_level.py# SQLAlchemy модель "Рівень користувача" (UserLevel) - зв'язок користувача з рівнем
│   │       │   │   ├── badge.py     # SQLAlchemy модель "Бейдж" (Badge), успадковує `BaseMainModel`
│   │       │   │   ├── achievement.py# SQLAlchemy модель "Досягнення користувача" (UserAchievement) - зв'язок user-badge
│   │       │   │   └── rating.py    # SQLAlchemy модель "Рейтинг користувача" (UserGroupRating)
│   │       │   │
...
│   │       │   │   ├── notification.py# SQLAlchemy модель "Сповіщення" (Notification)
│   │       │   │   ├── template.py  # SQLAlchemy модель "Шаблон сповіщення" (NotificationTemplate)
│   │       │   │   └── delivery.py  # SQLAlchemy модель "Статус доставки сповіщення" (NotificationDeliveryAttempt)
│   │       │   │
│   │       │   └── files/           # Каталог для моделей, пов'язаних з файлами
│   │       │       ├── __init__.py  # Ініціалізаційний файл пакету `files.models`
│   │       │       ├── file.py      # SQLAlchemy модель "Файл" (FileRecord), метадані файлу
│   │       │       ├── upload.py    # (Може бути частиною `FileRecord` або окремою логікою/сервісом)
│   │       │       └── avatar.py    # SQLAlchemy модель "Аватар користувача" (UserAvatar), зв'язок User-FileRecord
│   │       │
...
│   │       │   │   ├── settings.py  # Pydantic схеми для SystemSetting (SystemSettingCreate, SystemSettingUpdate, SystemSettingResponse)
│   │       │   │   ├── monitoring.py# Pydantic схеми для SystemLog, PerformanceMetric
│   │       │   │   └── health.py    # Pydantic схеми для ServiceHealthStatus, HealthCheckResponse
│   │       │   │
│   │       │   ├── auth/            # Pydantic схеми для аутентифікації та користувачів
│   │       │   │   ├── __init__.py  # Ініціалізація пакету `auth.schemas`
│   │       │   │   ├── user.py      # Pydantic схеми для User (UserCreate, UserUpdate, UserResponse, UserPublicProfile, UserSuperAdminView)
│   │       │   │   ├── token.py     # Pydantic схеми для Token (TokenResponse, RefreshTokenRequest)
│   │       │   │   └── login.py     # Pydantic схеми для Login (LoginRequest, PasswordResetRequest, PasswordResetConfirm)
│   │       │   │
...
│   │       │
│   │       ├── migrations/          # Каталог для міграцій бази даних (Alembic)
│   │       │   ├── __init__.py      # Ініціалізаційний файл пакету `migrations`
...
│   │       │   │   └── LC_MESSAGES/ # Стандартний каталог для gettext
│   │       │   │       └── messages.po # Файл перекладів для англійської мови у форматі .po
│   │       │   │       └── messages.mo # Скомпільований файл перекладів для англійської мови
│   │       │   └── uk/              # Каталог для української локалізації
│   │       │       └── LC_MESSAGES/ # Стандартний каталог для gettext
│   │       │           └── messages.po # Файл перекладів для української мови у форматі .po
│   │       │           └── messages.mo # Скомпільований файл перекладів для української мови
│   │       │
...
│   │   │
│   │   ├── unit/                    # Каталог для юніт-тестів
│   │   │   ├── __init__.py          # Ініціалізація пакету `unit` тестів
│   │   │   ├── test_models/         # Юніт-тести для SQLAlchemy моделей
